...
import { fetchProducts, fetchAllCarts, ApiProduct } from '../../lib/api';
...
const Carts: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setErrorMsg(null);
      try {
        const [prods, carts] = await Promise.all([fetchProducts(), fetchAllCarts()]);
        setProducts(prods);
        const mappedRows: AdminCart[] = carts.map((c: any) => {
          const mappedProducts = (c.items || []).map((it: { id_produit: number; quantite: number }) => {
      ...
      <div className="bg-gradient-to-r from-secondary to-accent rounded-lg p-6 text-white">
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="text-primary/90 mt-1">Tous les paniers (admin)</p>
          </div>
        </div>
      </div>
        ...
        <>
          ...
          <Card className="overflow-hidden">
            <div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
              ...
              <p className="text-sm text-gray-600 mt-1">Tous les produits par panier</p>
            </div>
